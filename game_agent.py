...
def difference_of_symmetric_set_difference(game: Board, player) -> float:
    ...
    """
    def get_moves_2_ahead(game, player):
    ...
    return (len(my_moves - opponent_moves) - len(opponent_moves - my_moves))


def fractional_3_deep(game: Board, player) -> float:
    """Calculate the heuristic value of a game state from the point of view
    of the given player.
    ...
    div = float(len(opponent_moves))
    return float("inf") if div == 0 else (len(my_moves)) / div


def fractional_2_deep(game: Board, player) -> float:
    """Calculate the heuristic value of a game state from the point of view
    of the given player.
...
def custom_score(game: Board, player) -> float:
    ...
    float
        The heuristic value of the current game state to the specified player.
    """
    return fractional_2_deep(game, player)
